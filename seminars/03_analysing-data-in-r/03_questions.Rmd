...
Update your code used to plot `my_plot` in Exercise 4 to have different fills for each condition and to have proper labels on the X and Y axis titles. The aesthetic mapping is where you should add the fill. Use the function `labs()` to add labels to plots.

## Exercise 6

In this experiment we want to exclude participants with average scores two standard deviations above or below the mean. Another way to think of this is as keeping only participants with scores greater than 2 standard deviation below the mean **and** less than 2 standard deviation above the mean. We will do this in parts.
...
Use the function `summarise()` to do this. Be sure to ungroup the data in your chain of functions first, e.g.: `data_summary %>% ungroup() %>% **your code goes here**`.
...
First, assign the lower and upper bounds for outliers to variables called "outlier_low" and "outlier_high".
...
Then filter out low and high scoring values in the original data using the variables above as your boundaries. Assign this to the variable "outliers".
...
Finally, remove any participant IDs who from the "outliers" variable who appear in your original data. Assign this to the object **data_summary_filtered**.
...
Make a code chunk. Taking your data from Exercise 6, conduct an appropriate statistical test to compare the mean scores for the congruent and incongruent conditions. Is there a significant difference between the groups?
