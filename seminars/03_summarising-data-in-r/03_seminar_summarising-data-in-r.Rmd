...
raw_data <- read_csv(here("seminar_three_data.csv"))
```
...
The label for the manipulation of the experiment and the two conditions is not very informative. Please give these suitable names in the data set.
...
Create a summary of the reaction times per condition for each participant. This will represent the participant averages in the experiment. Typically we use this data for inferential tests and plots.
...
Update the plot saved in `data_summary` to have different fills for each condition and to have proper labels on the X and Y axis titles.

## Exercise 6

In this experiment we only want to exclude participants with average scores one standard deviation above or below the mean.
