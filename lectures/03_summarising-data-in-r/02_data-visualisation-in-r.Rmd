...
Previously we found that it's difficult to see the pattern between height and mass due to an extreme outlier (Jabba the Hutt).

We can exclude this individual either by:

- Restricting limits on the axis of the plot using, e.g. `xlim()`.
...
One method to do this is using the Cartesian coordinate system. Here, we tell R to restrict the X-axis of this coordinate to be between 0 and 180 (kgs in mass).
