...
In this experiment we want to exclude participants with average scores two standard deviations above or below the mean.
...
First, assign the lower and upper bounds for outliers.

```{r exercise-6b-answer-part1}
outlier_low <- data_cutoffs$mean - data_cutoffs$sd*2
outlier_high <- data_cutoffs$mean + data_cutoffs$sd*2
```
...
outliers <- data_summary %>% 
  filter(mean_y < outlier_low | mean_y > outlier_high)
```
...
data_summary_filtered <- data_summary %>% 
  filter(!subj_id %in% outliers$subj_id)
```
...
```{r exercise-6c-2-answer, eval = FALSE}
...
```{r exercise-7-answer}
t.test(mean_y ~ A, data_summary_filtered, paired = TRUE)
```
...
sim_aov <- aov(outcome ~ condition, simdat)
summary(sim_aov)
```
