...
subtitle: "R Workshop 2 - Processing Data in R Questions"
output: 
  html_document:
    css: "../../style.css"
    toc: true
...
If you're unsure how these functions work, type `?mutate` and then `?case_when` in the console and press enter. The Help pane outlines how these functions work, and gives examples of working code.
...
Make a code chunk. Filter out any observations on the dependent variable which are NAs. Remember to use `is.na()` or `drop_na()`. Assign this new data to a object called "cleaned_data_three".
...
The ID column should be numeric. They do not need to be in any particular order or pattern. Can you make the IDs all numeric? For this, you want to **match** the **unique** ID's in the data set. You'll need the function `match()` and one other function to do this that you've seen before. 
...
The language column should have only two values (excluding NAs): English and Other. Can you change these to be a consistent code? Remember codes are case sensitive. Perhaps changing them to be lower case would help?
...
Are there any problems with the gender, age, and tester columns? Explain your thinking. You can use code to demonstrate your work. If there aren't, that's fine too!
...
We have two columns, funRec and completion_time, which actually store two variables each within a cell. 
...
Learn about the function, `separate()` from tidyr in the tidyverse. This will help to create two columns for each of these columns to keep our data separate. You can read about how `separate()` works by typing `?separate()` into the console and pressing enter.
...
Separate the data from completion_time into two columns, start and end. 

*Hint*: For this one, you'll need to be specific about what separates the two forms of data in the column.
