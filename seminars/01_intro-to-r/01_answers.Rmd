...
*You get a vector of strings containing "I'm using R" and "7". Because you can only store one data type within a vector, 7 is coerced from an integer to a string. This is because you can't sensibly go from a string to a number, but you can sensibly force a number to a string.*
...
```{r exercise-5-simplified-answer}
nums <- 1:10
...
```{r exercise-6-answer-part-a}
numbers <- c(1, 5, 10, 267)
numbers[4]
```

```{r exercise-6-answer-part-b}
numbers[4] <- 3
numbers
```
...
*This is too subjective to provide an answer key.*
...
*It has a space between < and - so assignment won't work (it will read this as less than then minus, but fail because sequence doesn't exist)*
...
*The name of the object has a typo.*
